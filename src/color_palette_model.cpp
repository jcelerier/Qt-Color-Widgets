...
public:
  ...
  bool acceptable(const QModelIndex& index) const { return acceptable(index.row()); }

  bool acceptable(int row) const { return row >= 0 && row <= palettes.count(); }

  QList<ColorPalette>::iterator find(const QString& name)
  {
    return std::find_if(palettes.begin(), palettes.end(), [&name](const ColorPalette& palette) {
  ...
  bool save(ColorPalette& palette, const QString& suggested_filename = QString())
  {
    ...
    if (!save_dir.exists(filename) && attemptSave(palette, save_dir.absoluteFilePath(filename)))
      return true;
    ...
    return attemptSave(
        palette, save_dir.absoluteFilePath(QString("%1%2.gpl").arg(palette.name()).arg(max + 1)));
  }
};
...
int ColorPaletteModel::indexFromFile(const QString& filename) const
{
  ...
  {
    if (!pal.fileName().isEmpty() && QFileInfo(pal.fileName()).canonicalFilePath() == canonical)
      return i;
    i++;
  }
  return -1;
