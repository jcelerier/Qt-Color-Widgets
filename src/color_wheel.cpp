...
public:
  ...
  qreal outer_radius() const { return qMin(w->geometry().width(), w->geometry().height()) / 2; }
...
ColorWheel::ColorWheel(QWidget* parent) : QWidget(parent), p(new Private(this))
{
...
void ColorWheel::paintEvent(QPaintEvent*)
{
  ...
  painter.drawEllipse(
      selector_position,
      color_widgets::detail::selector_radius,
      color_widgets::detail::selector_radius);
}
...
void ColorWheel::mouseMoveEvent(QMouseEvent* ev)
{
  ...
  else if (p->mouse_status == DragSquare)
  {
    ...
    QLineF center_mouse_ln(QPointF(0, 0), glob_mouse_ln.p2() - glob_mouse_ln.p1());
